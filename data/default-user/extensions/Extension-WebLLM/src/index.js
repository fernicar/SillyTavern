...
class WebLLMChatCompletionSource {
    static ID = 'webllm';
    ...
    get id() {
        return WebLLMChatCompletionSource.ID;
    }
        ...
        return [
            {
                ...
                name: 'Model',
                type: 'select',
                options: () => this.#engine.getModels().map(model => ({ value: model.id, name: model.toString() })),
                value: this.#engine.getCurrentModelInfo()?.id,
                onChange: (value) => this.#engine.loadModel(value),
            },
        ];
    }

    async generate(request) {
        const stream = this.#engine.generateChatStream(request.messages, request.options);
...
(function () {
    ...
    /**
     * @type {SillyTavernModule}
     */
    const extension = {
        onload: () => {
            const source = new WebLLMChatCompletionSource(defaultEngine);
            SillyTavern.registerChatCompletionSource(source);
        },
    };

    SillyTavern.addExtension('webllm', extension);
}());
